...
.factory('User', ['PageFunc', '$cordovaCamera', '$cordovaFileTransfer', 'CONFIG', '$timeout', 'Storage', 'Data', 'Token', '$state', '$ionicHistory', '$ionicModal', '$q', '$ionicSlideBoxDelegate', 'jwtHelper', function (PageFunc, $cordovaCamera, $cordovaFileTransfer, CONFIG, $timeout, Storage, Data, Token, $state, $ionicHistory, $ionicModal, $q, $ionicSlideBoxDelegate, jwtHelper) {
  ...
  self.dealPasswordModal = function ($scope, oldDealPwd, closeAble) {
    ...
    $scope.actions.dealPassword = function () {
      ...
      self.updateOnesPwd($scope.dealPassword).then(function (data) {
        ...
        if ($scope.actions.check) $scope.actions.check();  // 如果密码新增成功, 则执行check函数进行支付
  ...
  self.initInfo = function ($scope) {
    $scope.error = {};
    var deferred = $q.defer();
    if (Storage.get('info')) {  // 先显示localStorage中的值, 如果下面联机获取信息成功, 则更新账户信息, 否则就显示离线值(需要的话可以打印出错误信息)
        $scope.info = JSON.parse(Storage.get('info'));
        $scope.info.personalInfo.birthdate = new Date($scope.info.personalInfo.birthdate);
        // deferred.resolve(JSON.parse(Storage.get('info')));  // 因为$scope已经传入本服务, 可以直接对$scope.accountInfo赋值, 这里可以不用给resolve输入参数, 即可以deferred.resolve();
    }

    self.getInfo().then(function (data) {
        $scope.info = data.results;
        // $scope.info.user.head = {Url: 'img/userAvatar.jpg'};  // 测试用
        $scope.info.personalInfo.birthdate = new Date($scope.info.personalInfo.birthdate);
